...
**Status:** Fixed. `subcontractors` was routed as part of F-12 — the masked
list needs somewhere to fetch full details from. `workers` and `invoices` are
now routed too.

`createScopedCrud` scopes `getById` by `company_id`, so a foreign id and a
non-existent id both answer 404 and remain indistinguishable from outside.
That is why mounting these changes no contract: the endpoints previously 404'd
through the catch-all and now 404 through their own ownership check.

**Regression coverage:** two cases in `tests/tenantIsolation.test.js` — Beta
cannot read Alpha's worker by id, and Alpha can read its own. The second
matters as much as the first: without it the route could 404 for everyone and
the isolation test would still pass.
...
`GET /investors/:id/statement` and `GET /:master` now carry `requireOffice` of
their own. The effective roles are unchanged — `server.js` already mounted the
whole router behind it — so this is defence in depth rather than a policy
change.

What was wrong was that the file documented a gate it did not apply, leaving
the documentation true only because of a line in another file. Every route in
the module now states and enforces its own access.
